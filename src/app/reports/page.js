...
import { FileText, Calendar, Mail, FileOutput, RefreshCw, ChevronRight, CheckCircle2, Clock, ArrowLeft } from "lucide-react";
                ...
                >
                    <div>
                        ...
                        <p className="mt-2 text-lg text-slate-500">
                            Generate periodic summaries and track longitudinal progress.
                        </p>
                    </div>
                </motion.div>
                                ...
                                <div className="flex items-center gap-3 rounded-2xl bg-white/50 p-4 ring-1 ring-slate-200/50">
                                    ...
                                    <label className="flex flex-1 cursor-pointer items-center justify-between gap-2 text-sm font-medium text-slate-700">
                                        ...
                                        <div className="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus-within:ring-2 focus-within:ring-[#FDACAC] focus-within:ring-offset-2">
                                            ...
                                            />
                                            <div className={`h-6 w-11 rounded-full transition-colors ${sendEmail ? 'bg-[#FD7979]' : 'bg-slate-300'}`}></div>
                                            <span className={`absolute left-1 top-1 h-4 w-4 transform rounded-full bg-white transition-transform ${sendEmail ? 'translate-x-5' : 'translate-x-0'}`} />
                                        </div>
                                    </label>
                                </div>
                            ...
                            <AnimatePresence mode="popLayout">
                                {reports.length > 0 ? (
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <div className="flex w-full items-center justify-between sm:w-auto sm:flex-col sm:items-end sm:gap-2">
                                                    <div className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold shadow-sm ring-1 ${
                                                        report.status === "completed" 
                                                            ? "bg-emerald-50 text-emerald-600 ring-emerald-200/50" 
                                                            : "bg-amber-50 text-amber-600 ring-amber-200/50"
                                                    }`}>
                                                        {report.status === "completed" ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Clock className="h-3.5 w-3.5" />}
                                    ...
                                    >
                                        ...
                                        <p className="text-lg font-medium text-slate-600">No reports generated.</p>
                                        <p className="text-sm text-slate-400">Select a period and click Generate Document!</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </motion.div>
